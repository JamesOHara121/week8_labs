...
class Student(db.Model, UserMixin):
    ...
    student_id: so.Mapped[str] = so.mapped_column(db.String, index=True, unique=True)
...
class Group(db.Model):
    ...
    group_leader_id: so.Mapped[int] = so.mapped_column(sa.ForeignKey("student.id"), nullable=False)
...
class StudentGroup(db.Model):
    ...
    group_id: so.Mapped[int] = so.mapped_column(sa.ForeignKey("group.id"), nullable=False)
